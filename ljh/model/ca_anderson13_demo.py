...
# 3. (선택) 연료맵 시각화
def plot_fuel_map(fuel_array, title='Fuel Map'):
    plt.imshow(fuel_array, cmap='viridis', interpolation='nearest')
    plt.title(title)
    plt.colorbar(label='Fuel Index')
    plt.show()

# 4. Anderson13 코드별 번짐확률 예시
def get_fuel_spread_probs():
    return {
        'TL1': 0.10, 'TL2': 0.12, 'TL3': 0.13,
        'TU1': 0.18, 'TU2': 0.20, 'TU3': 0.22, 'TU4': 0.20, 'TU5': 0.25,
